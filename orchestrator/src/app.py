...
# Counters
order_counter = meter.create_counter("orders_received", description="Number of checkout requests received")
enqueued_counter = meter.create_counter("orders_enqueued", description="Number of orders enqueued")

# UpDownCounter to track concurrent checkouts
inflight_updown = meter.create_up_down_counter(
    "inflight_checkouts", description="Currently processing checkout flows"
)

# Histogram for processing durations (ms)
request_histogram = meter.create_histogram("checkout_processing_ms", description="Checkout processing time in ms")
...
def checkout():
    ...
    try:
        with tracer.start_as_current_span("checkout"):
            ...
            if final_resp.success:
                with grpc.insecure_channel("orderqueue:50054") as channel:
                    ...
                    if enqueue_response.enqueued:
                        enqueued_counter.add(1)
        ...
        request_histogram.record(duration_ms)
        inflight_updown.add(-1)
        _CURRENT_INFLIGHT -= 1
