...
# Instruments
enqueue_counter = meter.create_counter("queue_enqueued", description="Orders enqueued")
dequeue_counter = meter.create_counter("queue_dequeued", description="Orders dequeued")
queue_histogram = meter.create_histogram("queue_size_histogram", description="Queue size observed")

_QUEUE_REF = None

def _observe_queue(observable):
    if _QUEUE_REF is None:
        return
    observable.observe(len(_QUEUE_REF._queue))

meter.create_observable_gauge("order_queue_length", callbacks=[_observe_queue], description="Current order queue length")
...
class OrderQueueService(orderqueue_grpc.OrderQueueServiceServicer):
    ...
    def Enqueue(self, request, context):
        ...
        with tracer.start_as_current_span("enqueue"):
            ...
            enqueue_counter.add(1)
            queue_histogram.record(queue_size)
